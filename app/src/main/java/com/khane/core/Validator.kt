...
constructor(private val context: Context) {
    ...
    fun checkValidMobile(): MessageBuilder {
        ...
        for (builder in messageBuilders) {
            ...
            try {
                ...
                if (textInputLayout != null) {
                    ...
                    textInputLayout!!.error = e.getLocalizedMessage()
                }
    ...
    internal fun isEmpty(subject: String?, errorMessage: String?, byTrimming: Boolean) {
        var subjectStr: String? = subject ?: throw ApplicationException(errorMessage!!)

        if (byTrimming)
            subjectStr = subjectStr!!.trim { it <= ' ' }
    ...
    @Throws(ApplicationException::class)
    internal fun isValidPhone(subject: String, errorMessage: String?) {
    ...
    enum class Type {
        EMPTY, EMAIL, MIN_LENGTH, MAX_LENGTH, MATCH, PATTERN_MATCH, EMPTY_WITHOUT_TRIM, PHONE
    }
